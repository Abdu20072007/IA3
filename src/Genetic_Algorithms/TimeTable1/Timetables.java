...
public class Timetables {
    ...
    static Timetables[][][][] timetables = new Timetables[200][5][8][4];
    ...
    // cross over rate
    static double xoRate = 0.85;

    static String[] subjects = new String[8];

    static String[] classrooms = new String[6];

    static Teacher[] teachers = new Teacher[8];

    static Student[] students = new Student[40];
    ...
    public static Timetables[][][][] getTimetables() {
        return timetables;
    }

    public static String setTimetableSlot(String[] lesson, int randomNum) {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <= 199; i++) {
            ...
            for (j = 0; j < 5; j++) {
                ...
                for (h = 0; h < 8; h++) {
                    ...
                    for (k = 0; k < 4; k++) {
                        timetables[i][j][h][k] = new Timetables();
                        int randomNum = (int) (Math.random() * 8);
                        timetables[i][j][h][k].slot = setTimetableSlot(subjects, randomNum);
                        int randomNum2 = (int) (Math.random() * 6);
                        timetables[i][j][h][k].room = setSlotRoom(classrooms, randomNum2);
        ...
        System.out.println(timetables[1][2][3][3].slot);
        System.out.println(timetables[2][2][3][3].slot);

        // Fitness test

        for (int i = 0; i <= 199; i++) {
            fitness[i] = Fitness.subjectTest(timetables, i, subjects)
                    + Fitness.studentTest(timetables, students, i)
                    + Fitness.roomsTest(classrooms, timetables, i);
        ...
        for (int tournamentNo = 0; tournamentNo < 1000; tournamentNo++) {
            // pull 2 population members at random
            // a and b must be different
            a = (int) (200 * randomDouble());
            do {
                b = (int) (200 * randomDouble());
            } while (a == b);
                ...
                for (int j = 0; j < 7; j++) {
                    for (int k = 0; k < 3; k++) {
                        ...
                        if (randomDouble() < xoRate) {
                            timetables[Loser][i][j][k].slot = timetables[Winner][i][j][k].slot;
                            timetables[Loser][i][j][k].room = timetables[Winner][i][j][k].room;
                        }
                        // maybe do some mutation
                        if (randomDouble() < mutRate) {
                            timetables[Loser][i][j][k].slot = timetables[Loser][i][j][k].slot;
                            timetables[Loser][i][j][k].room = timetables[Loser][i][j][k].room;
                        }

                        fitness[Loser] = Fitness.subjectTest(timetables, Loser, subjects)
                                + Fitness.studentTest(timetables, students, Loser)
                                + Fitness.roomsTest(classrooms, timetables, Loser);

                        // then test to see if the new population member
                        // is a winner
                        if (fitness[Loser] == 0) {
                            System.out.println("Winner is: " + Loser);
                            System.exit(0);
                        }
                    }
                }
            ...
            if (tournamentNo == 999) {
                System.out.println("Winner is: " + Loser);
                System.out.println("Fitness is: " + fitness[Loser]);
                System.out.println("Tournament number is: " + tournamentNo);
            }
